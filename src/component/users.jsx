...
const Users=()=>{
    ...
    const handleDeliteUser = (id) => {
      setUsers((prevState) => prevState.filter(user => user._id !== id))
    } 

    const renderMassage = (number) => {
    ...
    return (
      ...
      <h2>
        ...
        >
        {users.length > 0 ? renderMassage(users.length) : 'Никто с тобой не тусанет!'}
        </span>
      </h2>
      {users.length > 0 && (
      <table
              ...
              users.map((user) => (
                <tr key={user._id}>
                  ...
                  <td>
                    ...
                    onClick={() => handleDeliteUser(user._id)}
                    >
                      delete
                    </button>
                  </td>
